...
pixidou = {
	...
	*/
	applyCropTool: function(){
		...
		var request = YAHOO.util.Connect.asyncRequest('GET', 'image.php?action=crop&image=' + pixidou.image + '&cropStartX=' + coordinates.left +'&cropStartY=' + coordinates.top +'&width=' + coordinates.width +'&height=' + coordinates.height + '&zoomLevel=' + pixidou.zoomLevel, {
			success: function(o){
	...
	*/
	applyRotateTool: function(direction, degrees){
		...
		if(pixidou.checkImage()){
			...
			// AJAX request
			var request = YAHOO.util.Connect.asyncRequest('GET', 'image.php?action=rotate&image=' + pixidou.image + '&direction=' + direction + '&degrees=' + degrees, {
				success: function(o){
					// update data
					pixidou.updateThruJson(o);
				},
	...
	*/
	applyFlipTool: function(direction){
		...
		if(pixidou.checkImage()){
			...
			// AJAX request
			var request = YAHOO.util.Connect.asyncRequest('GET', 'image.php?action=flip&direction=' + direction + '&image=' + pixidou.image + '&direction=' + direction, {
				success: function(o){
					pixidou.updateThruJson(o);
				},
	...
	*/
	applyResizeTool: function(){
		...
		// AJAX request
		var request = YAHOO.util.Connect.asyncRequest('GET', 'image.php?action=resize&image=' + pixidou.image + '&width=' + parseInt(YAHOO.util.Dom.getStyle('yuiImg', 'width'), 10) + '&height=' + parseInt(YAHOO.util.Dom.getStyle('yuiImg', 'height'), 10) + '&zoomLevel=' + pixidou.zoomLevel , {
			success: function(o){
				pixidou.updateThruJson(o);
	...
	*/
	applyNegativeTool: function(){
		...
		if(pixidou.checkImage()){
			...
			// AJAX request
			var request = YAHOO.util.Connect.asyncRequest('GET', 'image.php?action=negative&image=' + pixidou.image , {
				success: function(o){
					pixidou.updateThruJson(o);
				},
	...
	*/
	applyTintTool: function(color){
		...
		if(pixidou.checkImage()){
			...
			// AJAX request
			var request = YAHOO.util.Connect.asyncRequest('GET', 'image.php?action=tint&color=' + color + '&image=' + pixidou.image , {
				success: function(o){
					pixidou.updateThruJson(o);
				},
	...
	*/
	applyContrastTool: function(value){
		...
		if(pixidou.checkImage()){
			...
			// AJAX request
			var request = YAHOO.util.Connect.asyncRequest('GET', 'image.php?action=contrast&value=' + value + '&image=' + pixidou.image , {
				success: function(o){
					pixidou.updateThruJson(o);
				},
	...
	*/
	applyBrightnessTool: function(value){
		...
		if(pixidou.checkImage()){
			...
			// AJAX request
			var request = YAHOO.util.Connect.asyncRequest('GET', 'image.php?action=brightness&value=' + value + '&image=' + pixidou.image , {
				success: function(o){
					pixidou.updateThruJson(o);
				},
	...
	*/
	saveImage: function(type){
		...
		if(pixidou.checkImage()){
			...
			// AJAX request
			var request = YAHOO.util.Connect.asyncRequest('GET', 'image.php?action=save&type=' + type + '&image=' + pixidou.image , {
				success: function(o){
					// parse json data
